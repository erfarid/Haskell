...
--main =print(numtolist  12322)
sumOfSquaresOfDigits :: Int -> Int
sumOfSquaresOfDigits x =  sum[a^2 | a <- ls] 
 where 
  ls =numtolist x  
...
isPrime x 
 |x<2 =False
 |otherwise =length[a |a<-[1..x] ,x `rem` a==0]==2
...
countOddGap ls x = length[a |(a,i) <-zip ls [0,1..(length ls-3)] , a==x && odd(ls !!(i+2)) ]  
...
numtobinary 0 =[0]
numtobinary 1 =[1]
numtobinary x =  x `rem` 2 :numtobinary (x `div` 2)


--main =print(numtobinary  7)
isAllOnes :: Int -> Bool
isAllOnes x  = length[a | a<-ls,a==1] ==length ls 
 where
  ls =numtobinary x
...
selectiveMap fx fp ls  =[fp a | a <- ls , fx a ]
...
f1 :: String -> String
f1 ls  =[if(cheka6 a) then 'O' else a  | a <- ls ]    
...
absDiff lsa lsb n = nub[(a,b) | (a,b) <- zip lsa lsb  , abs(a-b)==n ]
...
averagegrade::[Int] -> Double 
averagegrade ls  = fromIntegral (sum ls )/ a
 where
  a =fromIntegral(length ls ) 

-- main =print(averagegrade [1,2,3,4,4])
averageScores :: [(String, [Int])] -> [(String, Double)]
averageScores ls  =[(a,averagegrade b ) |(a,b) <-ls]
...
filterTupleSum  a ls  = [ (x,y) | (x,y) <- ls ,(x+y) < a ]
...
findPairs ls  n  = nub[ (x,y) |x <- ls ,y <-ls  ,x < n && y >= n && even(x+y)]
...
generateFiltered st en b
  | st > en   = [] -- Return an empty list if start > end
  | otherwise = [a^2 | a <- [st..en], b && even a] ++ [a^3 | a <- [st..en], not b && odd a]
...
generate :: Char -> [String] -> [(Char, String)]
generate c birds = [(c, bird) | bird <- birds]

-- Function to check if a letter is already in the current library
isPresent :: [Char] -> Char -> Bool
isPresent lib c = c `elem` lib

-- Main function to generate all new letter codes and their possible name pairs
newGuess :: [Char] -> [(Char, String)]
newGuess codes =
  concat [generate code possibleBirds | code <- codes, not (isPresent currentLib code)]
