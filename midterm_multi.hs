...
--floor is used to make int from decimal 
--fromIntegral is used to convert int to decimal 
-- use div for integer 
--for decimal used  /
--in patternmatching always use this symbol :::
-- TASK 1 
{-
...
-}
--here count how many times the fdigit is eappeared one times 
...
helper2:: Int ->[Int] -> Int
helper2 x ls =length (filter(\xs-> xs==x) ls)


--main =print(helper2 2 [1,2,3,4,5,5,3,2,2,2])
--main =print(helper(1231)) need reverse 
countUniqueDigits :: Int -> Int
countUniqueDigits ls =length (filter (\a -> helper2 a (helper ls) == 1) (helper ls))
...
-- Function to check if the numbers at even positions are even
helpeven :: [Int] -> Bool
helpeven ls = and [even x | (x, i) <- zip ls [0..], even i]

-- Helper function to check if a number is prime
helpprime :: Int -> Bool
helpprime a = length [i | i <- [1..a], a `mod` i == 0] == 2

-- Check if numbers at odd positions are prime
primecheak :: [Int] -> Bool
primecheak ls = and [helpprime x | (x, i) <- zip ls [0..], odd i]

-- Count lists that satisfy prime check on odd positions and even check on even positions
count_good_lists :: [[Int]] -> Int
count_good_lists ls = length (filter (\x -> primecheak x && helpeven x) ls)
...
increaseByPosition ls =[x+y|(x,y)<-zip ls [0..]]
...
listtonum::[Int] ->Int
listtonum ls  =sum[x*(10^y) |(x,y)<-zip ls [a,a-1..0] ]
 where
    a=length ls-1

--main =print(listtonum[1,0,2])
--main =print(numtolist 12312)
rev_nums :: [Int] -> [Int]
rev_nums ls = map(\x->listtonum(reverse(numtolist(x)))) ls
...
passedStudents ls a =[s |(s,m)<-ls,sum (map (\x-> floor x) m) >=a]
...
--here i have to delete every third element 
...
carrylessDigitAddition la lb = [if a + b >= 0 then (a + b) `mod` 10 else -((- (a + b)) `mod` 10) | (a, b) <- zip la lb]
--carrylessDigitAddition la lb =[(a+b) `mod`10 |(a,b)<-zip la lb]
--carrylessDigitAddition la lb = [if sum >= 0 then sum `mod` 10 else (-((-sum) `mod` 10)) | (a, b) <- zip la lb, let sum = a + b]
...
-}
--least divisor of a number 
least::Int ->[Int]
least x =[a | a <- [2..x], x`mod`a ==0]

-- leastcommonsome::[Int] [Int] ->Int
-- leastcommonsome lsb lsb = maximum [a + b | a <- lsa, b <- lsb, a == b]


--main =print(least1 [2,5,10] [2,5,10,20])



gcdSubtraction :: Int -> Int -> Int
gcdSubtraction a b
    | a == b    = a  -- If the two numbers are equal, that's the GCD
    | a > b     = gcdSubtraction (a - b) b  -- Subtract the smaller number from the larger
    | otherwise = gcdSubtraction a (b - a)  -- Subtract the smaller number from the larger

--main =print(gcdSubtraction 15 10)

zipWithLCM :: [Int] -> [Int] -> [(Int, Int, Int)]
zipWithLCM lsa lsb =[(a,b ,a*b `div`gcd a b )|(a,b)<-zip lsa lsb ]
...
numtolist11::Int ->[Int]
numtolist11 0 =[] 
numtolist11 x = numtolist11(x `div`10)++[x`mod`10]

--main =print(numtolist11 2323)



listtonum11 :: [Int] -> Int
listtonum11 ls = sum [a * (10 ^ b) | (a, b) <- zip ls [length ls - 1, length ls - 2 .. 0]]

funNum :: Int -> Int
funNum x = listtonum11 (drop b lst ++ take b lst)
  where 
    lst = numtolist11 x
    a = length lst
    b = a `div` 2
...
foldIfTrue pred func ls
  |func =="max" = maximum (filter  pred  ls)
  |func =="min" =minimum(filter pred ls)
  |func =="+" = foldr (+) 0 (filter pred ls)
  |func =="*" =foldr (*) 1 (filter pred ls )
  |otherwise = error "wrong choice please provide correct choice"
...
averageSalary ls = fromIntegral (foldr(+) 0 fls) / fromIntegral(length fls)
 where 
  fls =[c | (a,b,c) <- ls]
...
--deducting salaeries by 15 percent muntiply it with 0.15
savedMoney :: [(String, Int, Int)] -> Double
savedMoney ls  =sum[m*0.15 |m<-b] 
 where  
    a = [(age,money) |(name,age,money)<-ls]
    b =[fromIntegral(money) |(age,money)<-a , age <35]
...
namesOlder35 ls = [ name|(name,age,saleries) <-ls,age>35,saleries>300]
