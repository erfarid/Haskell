...
f7 :: Int -> [Int] -> [Int]
--f7 n x = filter ((>) n) x  -- here i used the function form 
--f7 n x = filter (< n) x 
f7 n x =filter(\a -> a<n) x
 -- both are good
-- and here i used the operator form
...
f3 x =map(\x -> x `div` 2)(takeWhile (\a->a/=0) x ) 
...
checkAux list number =even(length(filter(\x-> x==number)list))

checkEven :: [Int] -> Bool
--checkEven list = and (map (checkAux list) list)
checkEven list = and(map(\x->checkAux list x)list) 
...
--main =print(group[1,1,2,2,2,3,3,3,3,3,3,4,4,4,4,4])
--group 
--group[1,1,2,2,2,3,3,3,3,3,3,4,4,4,4,4]  ->[[1,1],[2,2,2],[3,3,3,3,3,3],[4,4,4,4,4]]
--main = print(checkEven' [1,1,2,2,2,2,3,5,3,5]) -- True
--main = print(checkEven' [1,1,2,2,1]) -- False
--main = print(checkEven' []) -- True
